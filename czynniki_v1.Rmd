...
W analizie wykorzystano metody wizualizacji danych, klasyczne metody statystyczne (test t-Studenta i analiza wariancji ANOVA), model regresji liniowej, technikę selekcji zmiennych Lasso oraz analizę głównych składowych (PCA), która pomogła zrozumieć wzajemne zależności między zmiennymi. Całość pozwoliła na kompleksowe badanie zależności i identyfikację najistotniejszych czynników wpływających na wyniki egzaminów.
...
dane <- read.csv("C:/Users/kasia/Desktop/czynniki.csv", na.strings = c("NA", ""))
```
...
Dla uzupełnienia analizy przeprowadzono krótką analizę głównych składowych (PCA). Nie wnosi ona nowych wniosków do raportu – jej celem jest jedynie pokazanie, że zmienne numeryczne można pogrupować w ukryte czynniki, co stanowi opcjonalny krok eksploracyjny przed szczegółową analizą poszczególnych zmiennych.

```{r PCA}
# Wybór zmiennych numerycznych i przygotowanie danych
num_vars <- dane %>% select(where(is.numeric))
num_vars_complete <- na.omit(num_vars)
pca_res <- PCA(num_vars_complete, scale.unit = TRUE, graph = FALSE) # standaryzacja danych, mean = 0, sd = 1

# Scree plot – ile wariancji wyjaśniają składowe
fviz_eig(pca_res, addlabels = TRUE, ylim = c(0, 50)) +
    ggtitle("Scree plot - udział wariancji wyjaśnianej przez kolejne komponenty")
  
```

Na podstawie wykresu własnych wartości (scree plot) widać, że pierwsze 4 komponenty główne mają wartości własne większe od 1 i razem wyjaśniają ok. 69% całkowitej wariancji.


__Mapy zmiennych wyjaśnianych przez 4 główne komponenty__

Poniższe mapy zmiennych wizualizują, które zmienne grupują się razem.
...
* __PC1__ (24,7% wariancji) - Exam_Score, Hours_Studied, Attendance, w mniejszym stopniu Previous_Score i Tutoring_Sessions.

* __PC2__ (15% wariancji) – Hours_Studied, Previous_Scores, Sleep_Hours.

Na analizę obu składowych (PC1 i PC2) największy wpływ mają czynniki: Exam_Score, Attendance i Hours_Studied.

```{r pc3_pc4}
fviz_pca_var(pca_res, axes = c(3,4),
             col.var = "contrib",
             gradient.cols = c("blue", "white", "red"),
             repel = TRUE) +
      ggtitle("PC3 vs PC4")
...
* __PC3__ (14,6% wariancji) – Physical_Activity, Tutoring_Sessions, w mniejszym stopniu Hours_Studied.

* __PC4__ (14,5% wariancji) – Sleep_Hours, Physical_Activity, w mniejszym stopniu Hours_Studied i Attendance.

Na analizę obu składowych (PC3 i PC4) największy wpływ mają czynniki: Physical_Activity, Sleep_Hours.

Podsumowując, PC1 skupia głównie zmienne związane z wynikami egzaminu i frekwencją, PC2 – z nauką i snem, PC3 - z aktywnością fizyczną i nauką, a PC4 – z aktywnością fizyczną i snem. Analiza ta nie zmienia wniosków o wpływie zmiennych na wyniki egzaminu, ale pozwala lepiej zrozumieć ich wzajemne zależności.

# 5 Podsumowanie

Przeprowadzona analiza pozwoliła zidentyfikować kluczowe czynniki wpływające na wyniki uczniów w egzaminach. Dane poddano wstępnemu oczyszczeniu i imputacji braków, następnie dokonano wizualizacji zmiennych oraz analiz statystycznych, w tym testów t-Studenta, ANOVA, regresji liniowej, modelu Lasso. Dodatkowo przeprowadzono również analizę głównych składowych (PCA) dla zmiennych numerycznych jako krok eksploracyjny, bez wprowadzania nowych wniosków dotyczących wpływu na wyniki egzaminu.
